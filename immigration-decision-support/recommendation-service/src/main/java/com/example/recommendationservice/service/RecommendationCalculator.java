...
public class RecommendationCalculator {
    ...
    // demand * 0.4 + opportunities * 0.4 + (experience / 3) * 0.2
    // -------------------------------------------------------------------
    public BigDecimal calculate_profession_score(int demand, int experience, BigDecimal employmentOpportunities) {
        int cappedExperience = Math.min(experience, 30);

        BigDecimal demandPart = BigDecimal.valueOf(demand).multiply(BigDecimal.valueOf(0.4));
        BigDecimal oppPart = employmentOpportunities.multiply(BigDecimal.valueOf(0.4));
        BigDecimal expPart = BigDecimal.valueOf(cappedExperience)
                .divide(BigDecimal.valueOf(3), 4, RoundingMode.HALF_UP)
                .multiply(BigDecimal.valueOf(0.2));

        return demandPart.add(oppPart).add(expPart).setScale(1, RoundingMode.HALF_UP);
    }
    ...
    public BigDecimal calculate_personal_recommendation_score(UserProfileDto user, CountryDto country) {
        ...
        if (user.getProfessionId() != null) {
            try {
                ...
                demand  = cp.getDemandForProfession()  != null ? cp.getDemandForProfession()  : 0;
